...
1. **TypeScript 7.x (`tsconfig.json`)**:
   ...
   - Compile source `.ts` files into `dist/` via `tsc` (`outDir: "./dist"`), pointing `package.json` `"main"` to `dist/index.js`.
2. **ES Modules (ESM)**:
...
- **Negative:** Requires build step (`npm run build`) before distribution or local `npx` execution.
